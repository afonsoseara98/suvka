...
describe("buildReport", () => {
  it("returns null averages and no winner for a criterion with no scores at all", () => {
    ...
    expect(criterion.averages).toEqual({ noctra: null, chatgpt: null, claude: null, gemini: null });
    expect(criterion.winner).toBeNull();
  });

  it("computes per-source, per-criterion averages across multiple businesses", () => {
    const scores: ScoreRecord[] = [
      score({ businessId: "advogado", source: "noctra", criterion: "clarezaProposta", score: 8 }),
      score({ businessId: "dentista", source: "noctra", criterion: "clarezaProposta", score: 6 }),
      score({ businessId: "advogado", source: "chatgpt", criterion: "clarezaProposta", score: 5 }),
    ];
    const report = buildReport(scores);
    const criterion = report.perCriterion.find((c) => c.criterion === "clarezaProposta")!;
    expect(criterion.averages.noctra).toBe(7);
  ...
  it("picks the strict winner among all four sources for a criterion", () => {
    const scores: ScoreRecord[] = [
      score({ source: "noctra", criterion: "probabilidadeConversao", score: 9 }),
    ...
    const report = buildReport(scores);
    expect(report.perCriterion.find((c) => c.criterion === "probabilidadeConversao")!.winner).toBe("noctra");
  });

  it("lets a fourth source (gemini) win a criterion", () => {
    const scores: ScoreRecord[] = [
      score({ source: "noctra", criterion: "seo", score: 4 }),
  ...
  it("reports no winner (null) on an exact tie", () => {
    const scores: ScoreRecord[] = [
      score({ source: "noctra", criterion: "estruturaNarrativa", score: 7 }),
  ...
  it("computes unweighted overall averages per source across all criteria", () => {
    const scores: ScoreRecord[] = [
      score({ source: "noctra", criterion: "clarezaProposta", score: 10 }),
      score({ source: "noctra", criterion: "probabilidadeConversao", score: 6 }),
    ];
    const report = buildReport(scores);
    expect(report.overall.find((o) => o.source === "noctra")!.overallAverage).toBe(8);
  });
  ...
  it("computes a weighted overall average that favors high-weight criteria", () => {
    ...
    const scores: ScoreRecord[] = [
      score({ source: "noctra", criterion: "seo", score: 2 }),
      score({ source: "noctra", criterion: "probabilidadeConversao", score: 10 }),
    ];
    const report = buildReport(scores);
    const noctra = report.overall.find((o) => o.source === "noctra")!;
    expect(noctra.overallAverage).toBe(6);
    expect(noctra.weightedAverage).toBeCloseTo((2 * 0.5 + 10 * 1.5) / (0.5 + 1.5), 5);
    expect(noctra.weightedAverage!).toBeGreaterThan(noctra.overallAverage!);
  });

  it("flags every criterion Noctra does not strictly win as a loss, with a recommendation", () => {
    const scores: ScoreRecord[] = [
      score({ source: "noctra", criterion: "qualidadeVisual", score: 4 }),
      score({ source: "chatgpt", criterion: "qualidadeVisual", score: 8 }),
    ];
    const report = buildReport(scores);
    expect(report.noctraLosses).toHaveLength(1);
    expect(report.noctraLosses[0]).toMatchObject({
      criterion: "qualidadeVisual",
      winner: "chatgpt",
      noctraAverage: 4,
      winnerAverage: 8,
    });
    expect(report.noctraLosses[0].recommendation).toContain("VisualEngine");
  });

  it("does not flag a criterion Noctra wins, or a criterion with no data, as a loss", () => {
    const scores: ScoreRecord[] = [
      score({ source: "noctra", criterion: "clarezaProposta", score: 9 }),
      score({ source: "chatgpt", criterion: "clarezaProposta", score: 3 }),
    ];
    const report = buildReport(scores);
    expect(report.noctraLosses.find((l) => l.criterion === "clarezaProposta")).toBeUndefined();
    expect(report.noctraLosses.find((l) => l.criterion === "credibilidade")).toBeUndefined();
  });

  it("does not flag a tie as a loss", () => {
    const scores: ScoreRecord[] = [
      score({ source: "noctra", criterion: "confiancaTransmitida", score: 7 }),
      score({ source: "chatgpt", criterion: "confiancaTransmitida", score: 7 }),
    ];
    const report = buildReport(scores);
    expect(report.noctraLosses.find((l) => l.criterion === "confiancaTransmitida")).toBeUndefined();
  });
  ...
  it("gives every one of the 12 criteria a distinct, non-empty recommendation", () => {
    ...
    for (const criterion of BENCHMARK_CRITERIA) {
      const report = buildReport([
        score({ source: "noctra", criterion, score: 1 }),
        score({ source: "chatgpt", criterion, score: 9 }),
      ]);
      const loss = report.noctraLosses.find((l) => l.criterion === criterion)!;
